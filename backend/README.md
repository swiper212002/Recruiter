...
## Public API for Recruiter FE (optional)

The project exposes a small public API namespace `/api/public` that can be used by the recruiter frontend pages for quick question/test management without requiring authentication.

This API is disabled by default in production. To enable it locally or for development, set the environment variable `PUBLIC_API=true` or run in `NODE_ENV=development`.

Example (enable and start server):

```powershell
$env:PUBLIC_API='true'; npm run dev
```

Public endpoints (examples):
- `GET /api/public/questions` - list questions with options
- `POST /api/public/questions` - create question (payload: question_text, question_type, options[])
- `GET /api/public/tests` - list tests with questions
- `POST /api/public/tests` - create test (payload: test_name, duration_minutes, questions: [{question_id}])

Note: Public endpoints write `created_by: null` on created records. If you prefer authenticated flows, use the secured endpoints under `/api/questions` and `/api/tests` which require authentication.
