  ...
  createTest: async (req, res) => {
    ...
    try {
      ...
      if (questions && questions.length > 0) {
        // Deduplicate incoming questions to avoid false-negative existence checks
        const seen = new Set();
        ...
        if (duplicates.size > 0) {
          warnings.push({ type: 'duplicate_question_id', question_ids: Array.from(duplicates), action: 'ignored' });
        }

        // Validate questions (use unique list)
        const questionIds = uniqQuestions.map(q => q.question_id);
        logger.info(`Validating question IDs for test creation: ${JSON.stringify(questionIds)}`);
        const existingQuestions = await Question.findAll({ where: { question_id: { [Op.in]: questionIds } } });
        logger.info(`Found ${existingQuestions.length} existing questions for provided IDs`);
        ...
        // Add questions to test using the unique list
        await Promise.all(uniqQuestions.map((question, index) => {
